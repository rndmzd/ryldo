...
export const validateStreetAddress = (street) => {
    ...
    const commonWords = /(street|st|avenue|ave|road|rd|drive|dr|lane|ln|way|circle|cir|court|ct|boulevard|blvd|place|pl|square|sq|suite|ste|apartment|apt|unit|#)/i.test(street);

    if (!hasNumber) return { isValid: false, error: 'Address must include a street number' };
    if (!hasLetters) return { isValid: false, error: 'Address must include street name' };
    if (!validFormat) return { isValid: false, error: 'Address contains invalid characters' };
    if (!minLength) return { isValid: false, error: 'Address is too short' };
    if (!commonWords) return { isValid: false, error: 'Address must include a valid street type (St, Ave, Rd, etc.)' };
...
export const validatePostalCode = async (postalCode, country) => {
    try {
        const response = await fetch(`/api/validate/postal-code/${postalCode}?country=${country}`);
...
const verifyUSPSAddress = async (address) => {
    try {
        ...
        const token = await tokenManager.getToken('USPS', POSTAL_SERVICES.US.credentials);
        ...
        const response = await fetch(`${POSTAL_SERVICES.US.endpoint}/address?${params}`, {
            method: 'GET',
            headers: {
        ...
        if (response.status === 401) {
            ...
            const newToken = await tokenManager.getToken('USPS', POSTAL_SERVICES.US.credentials);
            const retryResponse = await fetch(`${POSTAL_SERVICES.US.endpoint}/address?${params}`, {
                method: 'GET',
                headers: {
...
const processUSPSResponse = async (response) => {
    if (!response.ok) {
        ...
        switch (response.status) {
            ...
            case 429:
                console.warn('Rate limit exceeded, retry after:', response.headers.get('Retry-After'));
                throw new Error('Rate limit exceeded');
            default:
                throw new Error(errorData.error.message || 'Address verification failed');
        }
    }
    ...
    return {
        ...
        standardizedAddress: {
            street: data.address.streetAddressAbbreviation || data.address.streetAddress,
...
export const verifyAddressWithPostalDB = async (address) => {
    try {
        ...
        if (!countryCode || countryCode !== 'US') {
            ...
            return { isValid: false, errors: { general: 'Only US addresses can be verified' } };
        }

        return await verifyUSPSAddress(address);
    } catch (error) {
        console.error('Address verification error:', error);
        return { isValid: false, errors: { general: 'Address verification failed' } };
    }
};
