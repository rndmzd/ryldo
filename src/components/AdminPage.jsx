...
import { getAllProducts, updateProductVisibility, updateProduct, login, logout } from '../services/api';
...
const validateUrl = (path, strict = false) => {
    ...
    if (!strict) {
        ...
        return !path.trim() || path.startsWith('/') || path.startsWith('http://') || path.startsWith('https://');
    }
...
const ImageManager = ({ product, onUpdate }) => {
    ...
    const handleUrlChange = (e) => {
        ...
        if (path && !validateUrl(path)) {
            setError('Image path must start with / for relative paths, or http:// or https:// for URLs');
        } else {
            setError('');
        }
    };
            ...
            <div className="space-y-4">
                <div className="space-y-2">
                    <div className="flex gap-2">
                        ...
                        />
                        <Button onClick={handleAddImage} disabled={!!error || !newImageUrl.trim()}>
                            Add Image
                        </Button>
                    </div>
                    {error && (
                    ...
                    <p className="text-sm text-gray-500">
                        Enter a relative path (e.g., /images/products/example.jpg) or full URL
                    </p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {images.map((url, index) => (
                        <div key={index} className="relative group aspect-square bg-gray-50 rounded-lg overflow-hidden">
                            <ImageWithFallback
                                src={url}
...
const VariantManager = ({ product, onUpdate }) => {
    ...
    const sizes = product.category === 'Drinkware'
    ...
    const handleAddVariant = (e) => {
        ...
        const newVariant = {
            ...
            color: product.category !== 'Drinkware' ? formData.get('color') : undefined,
    ...
    return (
        <div className="p-4">
            ...
            {showAddForm && (
                <form onSubmit={handleAddVariant} className="mb-4 p-4 border rounded-lg space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium mb-1">Size</label>
                            <select name="size" className="w-full p-2 border rounded" required>
                                {sizes.map(size => (
                        ...
                        {product.category !== 'Drinkware' && (
                            <div>
                                ...
                                <select name="color" className="w-full p-2 border rounded" required>
                                    {colors.map(color => (
                    ...
                    <div key={index} className="flex items-center justify-between p-2 border rounded">
                        <div className="flex-1">
                            ...
                            <p className="text-sm text-gray-600">
                                ${variant.price.toFixed(2)} - {variant.inStock ? 'In Stock' : 'Out of Stock'}
                            </p>
                        </div>
                        <div className="flex gap-2">
                            <Button
                                variant="outline"
                                size="sm"
                                onClick={() => handleUpdateVariant(index, { inStock: !variant.inStock })}
                            >
            ...
            >
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-gray-600">{product.category} - ${product.price.toFixed(2)}</p>
                    </div>
                </div>
                ...
                <div className="mt-4">
                    ...
                    {isEditing ? (
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium mb-1">Main Image Path/URL</label>
                                <input
                                    type="text"
                    ...
                    <div className="mt-4 space-y-4">
                        ...
                        <ImageManager
                            product={product}
                            onUpdate={(images) => onUpdate({ ...product, additionalImages: images })}
                        />
                    </div>
                </div>
            )}
